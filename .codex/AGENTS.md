...
- 常に英語で思考し、日本語で応える
- 箇条書きをする場合、今後のやり取りをしやすいように数字の箇条書きにする
- 効率を最大限に高めるために複数の独立した操作を実行する必要がある場合は、関連するすべてのツールを順番にではなく同時に呼び出します。
...
## Github Issues/PullRequestの操作について

- ユーザーからIssueやPull Request(PR)の操作を依頼された場合、ghコマンドを使って操作をしてください。
- Read操作に関しては特に確認の必要はありません
- Write/Delete操作に関しては、実施前に**必ず**確認してください
- IssueやPull Requestを作成する際は、日本語で作成してください
